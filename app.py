...
def fetch_kma_ultra():
    ...
    for b in candidates:
        p = {
            ...
            "base_date": b.strftime("%Y%m%d"),"base_time":b.strftime("%H%M"),
            "nx":nx,"ny":ny
        }
        try:
            ...
            for it in items:
                ...
                if cat in ("T1H","REH"):
                    try: bucket[ts][cat]=float(val)
                    except: pass

            with app.app_context():
                for ts,v in bucket.items():
                    t,h=v["T1H"],v["REH"]
                    if t is None and h is None: continue
                    ...
                    if row:
                        ...
                        db.session.add(WeatherReading(source="KMA10",temperature=t,humidity=h,pressure=None,timestamp=ts))
                    db.session.commit()
            break
        except: pass
...
def fetch_kma_asos():
    ...
    try:
        ...
        h=header.split(); idx={h[i]:i for i in range(len(h))}
        ti,ta,hm,pa=idx["YYMMDDHHMI"],idx["TA"],idx["HM"],idx["PA"]

        def conv(v): return None if v in ["","-999.0","-999"] else float(v)

        with app.app_context():
            for row in data:
                p=row.split()
                ts=datetime.strptime(p[ti],"%Y%m%d%H%M").replace(tzinfo=KST)
                t,h,pres=conv(p[ta]),conv(p[hm]),conv(p[pa])
                r2=WeatherReading.query.filter_by(source="KMA",timestamp=ts).first()
                if r2:
                    if t is not None: r2.temperature=t
                    if h is not None: r2.humidity=h
                    if pres is not None: r2.pressure=pres
                else:
                    db.session.add(WeatherReading(source="KMA",temperature=t,humidity=h,pressure=pres,timestamp=ts))
                db.session.commit()
    except:
        pass
        ...
        try:
            ...
            while True:
                ...
                if minute!=last_min:
                    with app.app_context():
                        try:
                            db.session.add(WeatherReading(source="Arduino",temperature=t,humidity=h,pressure=p,timestamp=minute))
                            db.session.commit()
                            last_min=minute
...
def chart(cat):
    field = {"temperature":"temperature","humidity":"humidity","pressure":"pressure"}.get(cat)
    if not field:
    ...
    first=WeatherReading.query.filter(
    ...
    labels_aw=ten_min_range(start,now)
    ...
    if cat=="pressure":
        kma_recs=WeatherReading.query.filter(
            ...
            WeatherReading.timestamp.between(start,now)
        ).order_by(WeatherReading.timestamp).all()
    else:
        kma_recs=WeatherReading.query.filter(
            WeatherReading.source=="KMA10",
            WeatherReading.timestamp.between(start - timedelta(hours=2),now)
        ).order_by(WeatherReading.timestamp).all()
